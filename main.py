...
TOKEN = '7754606350:AAFRwv7-tXmYYesaxvSdgb9Y7_fHHRl9mHU'
bot = TeleBot(TOKEN)
game = False
night = False
...
def game_loop(message):
    ...
    while True:
        ...
        if night:
            ...
            bot.send_message(message.chat.id, 'Город засыпаерт, просыпается мафия')
...
def game_on(message):
    if not game:         
        bot.send_message(message.chat.id, text='Если хотите играть напишите "готов играть" в лс')
...
def game_start(message):
    ...
    if players >=5 and not game:
        ...
        game = True
        bot.send_message(message.chat_id, text='Игра началась')
        return 
    bot.send_message(message.chat_id, text='Не достаточна игроков ')
...
def kick(message):
    ...
    if not night:
        ...
        if voted:
            ...
            return
        bot.send_message(message.chat.id, 'У вас больше нет праваголосовать')
        return
