...
db = r'C:\Users\123\Documents\piton\mafia_bot\db.db'

def insert_players(player_id, username):
...
def check_winner():
    con = sqlite3.connect(db)
    cur = con.cursor()
    cur.execute(f"SELECT COUNT(*) FROM players WHERE role = 'mafia' and dead = 0")
    mafia_alive = cur.fetchone()[0]
    cur.execute(f"SELECT COUNT(*) FROM players WHERE role != 'mafia' and dead = 0")
